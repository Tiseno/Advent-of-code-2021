 ...
 Paths allPaths(
	...
	for (auto next : graph[node]) {
		if (visited.find(next) == visited.end()) {
			...
			auto newPaths = allPaths(newPath, newVisited, next, graph, indexToName, smallCaves);
...
int main() {
	...
	for (size_t i = 0; i < graph.size(); i++) {
		cout << "Node " << i << " " << indexToName.find(i)->second << "\t";
		cout << " -> ";
		for (auto q : graph[i]) {
			cout << q << " ";
		}
		if (smallCaves.find(i) != smallCaves.end()) {
			cout << "(small)";
		}
		cout << endl;
	}
	...
	auto paths = allPaths(Path(), Visited({ start }), start, graph, indexToName, smallCaves);
